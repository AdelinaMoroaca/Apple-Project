...
function Header(props) {
  const { bgThemeVariant, iconColor } = props;
  const { user, setUser } = useContext(AuthenticationContext);
  const [expanded, setExpanded] = useState(false);
  ...
  return (
    <header>
      <Navbar bg={bgThemeVariant} className={styles.navbarContainer} data-bs-theme={bgThemeVariant} expand="lg" onToggle={() => setExpanded(!expanded)}>
        <Container className="d-flex justify-content-between align-items-center" fluid>
          {/* Left section for small screens */}
          <div className="d-lg-none ">
            <Navbar.Brand as={Link} to="/">
              <FontAwesomeIcon icon={faApple} size="md" color={iconColor}/>
            </Navbar.Brand>
          </div>
          ...
          <div className="d-none d-lg-flex justify-content-center align-items-center flex-grow-1">
            <Navbar.Brand as={Link} to="/" className="me-4">
              <FontAwesomeIcon icon={faApple} size="md" />
            </Navbar.Brand>
            <Nav>
              ...
              <Nav.Link as={Link} to="/shop/signIn" onClick={logoutUser}  className={`${styles.navLink} d-none d-lg-block`} >
                {user ? (
                  <>
                  <FontAwesomeIcon icon={fasUserCircle} size="md"/>
                  <span> Sign Out TEST</span>  
                  </>        
                ): <FontAwesomeIcon icon={farUserCircle} size="md"/> } 
              </Nav.Link>
            </Nav>
          </div>

            {/* For small screens: Search, Cart, and Burger Menu */}
            <div className={`d-lg-none d-flex align-items-center`}>
              {/* {need to implement searchbar} */}
              {/* <Nav.Link as={Link} to="/search" className="d-none d-lg-block">
                <FontAwesomeIcon icon={faSearch} size="md" color={iconColor} />
              </Nav.Link> */}
              <Nav.Link as={Link} to="/bag" className="d-none d-lg-block">
                <FontAwesomeIcon icon={faShoppingBasket} size="md"/>
              </Nav.Link>

              {user ? (
                <Nav.Link as={Link} to="/favorites" className={styles.navLink} >
                  <FontAwesomeIcon icon={farHeart} size="lg" color={iconColor}/>
                </Nav.Link>
              ): '' } 
              ...
              <Nav.Link as={Link} to="/shop/signIn" onClick={logoutUser}>
                {user ? (
                  <>
                  <FontAwesomeIcon icon={fasUserCircle} size="md" color={iconColor} />
                  <span> Sign Out Adelina</span>  
                  </>        
                ): <FontAwesomeIcon icon={farUserCircle} size="md" color={iconColor} /> } 
              </Nav.Link>

              <Nav.Link as={Link} to="/bag" className={styles.navLink}>
                <FontAwesomeIcon icon={faShoppingBasket} size="md" color={iconColor} />
              </Nav.Link>
              <Button
      ...
      <Collapse in={!collapsed}>
        <div id="navbar-collapse" 
            className={`bg-${bgThemeVariant} text-${bgThemeVariant === 'dark' ? 'light' : 'dark'} ${styles.toggleStyle}`}
            data-bs-theme={bgThemeVariant} 
...
// export { HeaderBlack, HeaderWhite, Header };
export { Header };